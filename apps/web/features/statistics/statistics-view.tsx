...
import { StatisticsActivityVisual } from "./statistics-activity-visual";
import {
  StatisticsChart,
...
export function StatisticsView() {
  ...
  const periodHighlight = useMemo(
    () => getPeriodHighlightActivity(summary.periodActivities) ?? latestActivity,
    [latestActivity, summary.periodActivities],
  );
  const weekdayCounts = useMemo(
    () => getWeekdayActivityCounts(summary.periodActivities),
    [summary.periodActivities],
  );
  const maxWeekdayCount = Math.max(...weekdayCounts, 1);
  const reliefPoints = summary.chartData.slice(-10).map((datum) => datum.elevation);
  const maxReliefPoint = Math.max(...reliefPoints, 1);
  const topTerrains = distribution.slice(0, 3);
          ...
          <>
            <section className={styles.hero} aria-labelledby="statistics-title">
              ...
              <div className={styles.heroStory}>
                {periodHighlight ? (
                  ...
                  >
                    <StatisticsActivityVisual
                      activity={periodHighlight}
                      eyebrow="Trace de la période"
                      placement="hero"
                      visualSource="hovren"
                    />
                  </Link>
              ...
              <div className={styles.insightGrid}>
                <article className={styles.insightLead}>
                  ...
                  <div>
                    ...
                    <div className={styles.insightMiniBars} aria-hidden="true">
                      {reliefPoints.map((value, index) => (
                        <i
                          key={`${value}-${index}`}
                          style={{ height: `${Math.max(10, (value / maxReliefPoint) * 100)}%` }}
                        />
                      ))}
                    </div>
                  </div>
                ...
                <article>
                  ...
                  <div>
                    ...
                    <div className={styles.insightMiniRhythm} aria-hidden="true">
                      {weekdayCounts.map((count, index) => (
                        <i key={`${count}-${index}`}>
                          <span style={{ height: `${Math.max(8, (count / maxWeekdayCount) * 100)}%` }} />
                        </i>
                      ))}
                    </div>
                  </div>
                ...
                <article>
                  ...
                  <div>
                    ...
                    <div className={styles.insightMiniTerrain} aria-hidden="true">
                      {topTerrains.map((terrain) => (
                        <i key={terrain.sport}>
                          <span>{terrain.label}</span>
                          <b style={{ width: `${terrain.percent}%` }} />
                        </i>
                      ))}
                    </div>
                  </div>
            ...
            >
              ...
              <StatisticsChart
                ...
                metric={metric}
                activities={summary.periodActivities}
              />
            </section>

            <div className={styles.rhythmGrid}>
              <StatisticsCalendar activities={completedActivities} />
                ...
                <div className={styles.recordList}>
                  ...
                  >
                    ...
                    <span className={styles.recordCopy}>
                      ...
                      <strong>{formatDistance(longestActivity?.distance)}</strong>
                      <span>{getActivityTitle(longestActivity)}</span>
                      <ArrowRight aria-hidden="true" />
                    </span>
                  </Link>
                  <Link
                  ...
                  >
                    ...
                    <span className={styles.recordCopy}>
                      ...
                      <strong>{formatElevation(climbingActivity?.elevationGain)}</strong>
                      <span>{getActivityTitle(climbingActivity)}</span>
                      <ArrowRight aria-hidden="true" />
                    </span>
                  </Link>
                  <Link
                  ...
                  >
                    {altitudeActivity ? (
                      <StatisticsActivityVisual
                        activity={altitudeActivity}
                        eyebrow="Point culminant"
                        compact
                      />
                    ) : null}
                    <span className={styles.recordCopy}>
                      ...
                      <strong>{formatElevation(altitudeActivity?.maxAltitude)}</strong>
                      <span>{getActivityTitle(altitudeActivity)}</span>
                      <ArrowRight aria-hidden="true" />
                    </span>
                  </Link>
                </div>
