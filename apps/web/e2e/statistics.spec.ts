...
test("le bilan reste fonctionnel et éditorial sur desktop", async ({ page }) => {
  ...
  const calendarDayCount = await page
    .locator('[class*="calendarDayWrap"]')
    .count();
  expect([35, 42]).toContain(calendarDayCount);
  await expectNoHorizontalOverflow(page);
  await page.screenshot({
    path: "/tmp/hovren-statistics-desktop.png",
  });

  const highlightTitle = page
    .getByRole("link", { name: "Ouvrir la trace marquante" })
    .getByText("Traversée des Bornes", { exact: true });
  const highlightBounds = await highlightTitle.boundingBox();
  const metricsBounds = await page
    .getByText(/Distance parcourue/)
    .first()
    .locator("..")
    .boundingBox();
  ...
  expect(highlightBounds!.y + highlightBounds!.height).toBeLessThan(
    metricsBounds!.y - 8,
  );
...
test("le bilan reste compact et sans débordement sur mobile", async ({
  ...
  const mobileCalendarMonth = page.getByTestId("statistics-calendar-month");
  ...
  await expectNoHorizontalOverflow(page);
  await page.screenshot({
    path: "/tmp/hovren-statistics-calendar-mobile.png",
  });
});
